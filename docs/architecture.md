...
Each hook performs bounded local work: read and filter the payload, identify the process, assign an event ID and sequence, atomically spool the event, and attempt an immediate bounded flush. Proposed network budget: one second, with the whole hook returning comfortably inside the existing installer's three-second Codex timeout. No long retries occur in the hook. It prints no stdout and exits successfully on reporting failure so it cannot alter an agent's permission or execution decision.
...
A scheduled invocation of the same script retries the outbox even if no further hooks fire. Proposed schedule: every 30 seconds on Linux via a user timer, with a cron option at lower resolution. This intentionally changes the README's no-polling promise. Hooks-only operation remains possible, with retries on the next hook or manual flush and weaker liveness guarantees.
...
**Retention and evolution.** Proposed defaults: sanitized lifecycle events for 30 days, ended-run summaries and execution high-water marks/tombstones for 90 days, usage evidence for 90 days, and derived daily summaries for one year. Active run state persists while tracked. Keep minimal event-ID/hash deduplication records at least through the permitted seven-day retry horizon after pruning payloads. Reject expired replays and events targeting expired closed executions so pruning cannot resurrect a session. Daily aggregates are replaceable materializations with explicit timezone and pricing versions; detailed repricing/reconstruction is limited by retained evidence.

Use a scheduled Worker for bounded retention work. Estimate event volume and index overhead before choosing a Cloudflare plan; D1 has finite per-database capacity and serial query execution, documented in its [platform limits](https://developers.cloudflare.com/d1/platform/limits/). Monitor accepted/rejected/duplicate events, ingestion lag, stale installations, dropped local events, query duration, and storage growth. Add archival or partitioning based on observed demand.
