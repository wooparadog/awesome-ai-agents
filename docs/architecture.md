...
The reporter is a Rust daemon using scoped bearer tokens, an event journal plus current-session tables, and a hibernating WebSocket connection for the widget. WebSockets with hibernation are an accepted design decision. Preserving all existing behavior also needs local process and transcript observations; that work is explicitly included below.

```mermaid
flowchart LR
  A[Agent hooks on each machine] --> H[Rust daemon]
  T[Internal reconciliation deadlines] --> H
...
**Rust reporter.** Hooks invoke `ai-agents hook <agent> <event>`. This command
allowlists metadata, durably writes a small inbox item, and wakes the daemon over
a private Unix socket. The daemon owns identities, sequence assignment, local
process observations, incremental transcript parsing, and batched HTTP delivery.
One current-thread async runtime and one in-flight request keep resource use
bounded; network waits do not block hook intake. No child shell utilities run.

A write-ahead transaction protects identity updates and inbox consumption. Usage
batches reach disk before cursor advancement. Retry IDs, run IDs, and transcript
cursor keys are compatible with existing shell state. Retry delays survive
restarts and honor server backoff. Historical events cannot renew presence; each
presence request uses a fresh process fingerprint.

The daemon checks local processes and transcript growth every 30 seconds, with
immediate work on hook wakeups. Unchanged live-process presence renews every five minutes; empty idle reporters stay silent;
unchanged local scans make no HTTP requests. It can run under any supervisor. The
installer optionally creates one systemd service and disables the legacy timer
and path watcher. See the [daemon guide](../reporters/rust/README.md) for resource
bounds, installation, migration, and validation.
...
Clearing `asking` also requires a signal. Prefer an explicit supported hook; otherwise reconciliation compares transcript activity against the checkpoint captured when attention was requested. Transcript growth can provide the same heuristic the current widget uses, but is not proof of user approval. Mark the observation's reason/confidence. The daemon checks this condition every 30 seconds locally and on hook wakeups.

**Usage and costs.** The Worker cannot open a machine's transcript path. Preserving this feature therefore requires a local metadata extractor, invoked outside the hook's time budget, using the daemon’s Rust transcript adapters. Incremental offsets and parsing checkpoints remain local. Upload only usage IDs, timestamps, model IDs, and counters; keep prompts and generated text on the machine. Read complete JSONL records in bounded chunks, tolerate file rewrites/rotation, and advance durable parsing state only once the extracted records are durably spooled.
...
Authorization is checked at upgrade and on every snapshot request. Keep idle sockets for up to one day, limited by token expiry. Revalidate authorization before publishing after the five-minute cached check expires. Store the deadline in its attachment, filter expired sockets before broadcasts, and use a durable alarm to close them while idle. Token revocation also attempts immediate closure through the workspace object; revalidation before publication bounds information delivery when that closure fails. One object has one alarm, so persist and schedule the earliest pending notification retry or authorization expiry and process due jobs together. These bounded wakeups allow hibernation between events; they do not promise zero activity or zero charges.
...
Accepted decisions: viewer clients use WebSockets with Durable Object hibernation; the Linux reporter uses one Rust daemon with internal reconciliation and retry deadlines; the AwesomeWM client uses GLib/libsoup; attention clearing may take a reconciliation interval. The legacy shell implementation remains available for rollback.
